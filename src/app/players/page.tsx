...
export default function PlayersPage() {
  ...
  const handleEdit = (playerId: number, field: string, value: string) => {
    setEditingCell({ playerId, field, value });
    setEditValue(value);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('Mise à jour:', { ...editingCell, newValue: editValue });
      setEditingCell(null);
    } catch (error) {
              ...
              <tbody className="divide-y divide-gray-200">
                {players.map((player) => (
                  <tr key={player.id}>
                    ...
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{player.victories}</td>
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{player.defeats}</td>
                    ...
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{Math.floor(player.congo)}</td>
                    <td className="whitespace-nowrap px-3 py-4">
                      <button
